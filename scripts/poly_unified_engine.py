...
def run_engine():
    ...
    if current_price >= 0.85:
        print(f"  🚨 动作触发: 达到止盈线 ($0.85)! 建议立刻卖出 50% 仓位!")
    elif current_price <= 0.40:
        print(f"  🚨 动作触发: 跌破止损线 ($0.40)! 建议清仓!")
    elif score >= 60 and current_price < 0.85:
        print(f"  🚨 动作触发: OSINT 概率飙升至 {score}%，但盘面尚未反应 ($current_price)。建议加仓!")
    else:
        print(f"  🟢 综合评判: 盘口与基本面静默，系统持续蛰伏中。")
        print("TEST_PULSE_OK")
