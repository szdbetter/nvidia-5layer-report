...
# 默认使用 VolcEngine Seedream；可通过 SEEDREAM_MODEL 环境变量覆盖
# doubao-seedream-4-0 同时支持 t2i 和 i2i，分辨率需 ≥921600 像素
...
# VolcEngine 使用像素尺寸而非 2K/3K 字符串
VOLCENGINE_RESOLUTION_MAP = {
    "2K": "1920x1080",
    "3K": "2880x1620",
}
...
def load_easyclaw_runtime_config(state_dir: Path) -> tuple[str, str, str]:
    """
    优先读取环境变量，找不到再回退到 easyclaw 配置文件。
    环境变量：
      SEEDREAM_BASE_URL  — API 端点（如 https://ark.cn-beijing.volces.com/api/v3）
      SEEDREAM_UID       — X-Auth-Uid 头
      SEEDREAM_TOKEN     — X-Auth-Token 头（或直接用 VOLCENGINE_API_KEY）
    """
    import os

    env_base_url = os.environ.get("SEEDREAM_BASE_URL", "").strip().rstrip("/")
    env_uid = os.environ.get("SEEDREAM_UID", "").strip()
    env_token = (
        os.environ.get("SEEDREAM_TOKEN")
        or os.environ.get("VOLCENGINE_API_KEY")
        or ""
    ).strip()

    # 优先：显式 env 变量
    if env_base_url and env_token:
        return env_base_url, env_uid, env_token

    # 次选：自动使用 VolcEngine（VOLCENGINE_API_KEY 已配置即可）
    volcengine_key = env_token  # 上面已经 fallback 过 VOLCENGINE_API_KEY
    if volcengine_key:
        return VOLCENGINE_BASE_URL, "", volcengine_key
...
def build_openai_client(base_url: str, uid: str, token: str) -> OpenAI:
    # 有 uid 时使用 easyclaw 风格头；否则使用标准 Bearer token
    if uid:
        return OpenAI(
...
def build_image_request_args(args: argparse.Namespace, output_resolution: str, is_volcengine: bool = False) -> dict:
    import os
    model = os.environ.get("SEEDREAM_MODEL", MODEL_NAME)

    # VolcEngine 用像素尺寸，easyclaw 用 2K/3K 字符串
    size = VOLCENGINE_RESOLUTION_MAP.get(output_resolution, output_resolution) if is_volcengine else output_resolution
    ...
    # easyclaw 支持 output_format，VolcEngine 默认 jpeg 无需指定
...
def main() -> int:
    ...
    try:
        ...
        is_volcengine = (base_url == VOLCENGINE_BASE_URL)
        ...
        request_args = build_image_request_args(args, output_resolution, is_volcengine=is_volcengine)
